...
class IpRegion
{
    ...
    */
    public function memorySearch($ip)
    {
        ...
        if ($this->dbBinStr == null) {
            ...
            $this->totalBlocks   = ($this->lastIndexPtr-$this->firstIndexPtr) / IpRegion::INDEX_BLOCK_LENGTH + 1;
        }
        if (is_string($ip)) {
        ...
        while ($l <= $h) {
            $m = (($l + $h) >> 1);
            $p = $this->firstIndexPtr + $m * IpRegion::INDEX_BLOCK_LENGTH;
    ...
    */
    public function binarySearch($ip)
    {
        ...
        while ($l <= $h) {
            $m = (($l + $h) >> 1);
            $p = $m * IpRegion::INDEX_BLOCK_LENGTH;
            fseek($this->dbFileHandler, $this->firstIndexPtr + $p);
            $buffer = fread($this->dbFileHandler, IpRegion::INDEX_BLOCK_LENGTH);
    ...
    */
    public function btreeSearch($ip)
    {
        ...
        if ($this->HeaderSip == null) {
            ...
            $buffer = fread($this->dbFileHandler, IpRegion::TOTAL_HEADER_LENGTH);
            
            //fill the header
            $idx = 0;
            $this->HeaderSip = [];
            $this->HeaderPtr = [];
            for ($i = 0; $i < IpRegion::TOTAL_HEADER_LENGTH; $i += 8) {
        ...
        $index = fread($this->dbFileHandler, $blockLen + IpRegion::INDEX_BLOCK_LENGTH);
        
        $dataPtr = 0;
        $l = 0;
        $h = $blockLen / IpRegion::INDEX_BLOCK_LENGTH;
        while ($l <= $h) {
            $m = (($l + $h) >> 1);
            $p = (int)($m * IpRegion::INDEX_BLOCK_LENGTH);
